...
### v1.2.0 - 多记录JSON支持
- 🚀 **新增多记录JSON格式**：在单个域名中编码多种DNS记录类型
- 📊 **分层域名支持**：支持长JSON数据的多层编码（j1.j2.j3格式）
- 🔧 **扩展记录类型**：新增DNAME、TLSA、SSHFP、NAPTR、HINFO、LOC等记录类型支持
- ✅ **完整测试覆盖**：为新功能添加了全面的测试用例
- 📖 **文档更新**：完整的多记录功能使用指南
...
- 多种编码格式:
  ...
  - **新功能**: 多记录JSON格式（在单个域名中编码多种DNS记录类型）
- 全面的DNS记录类型支持（A、AAAA、TXT、MX、SRV、CNAME等多种类型）
- CSV文件支持传统DNS记录
- 通配符记录支持
